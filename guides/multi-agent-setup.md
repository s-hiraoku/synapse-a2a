...
| 自分の専門外、READY なエージェントが存在 | `synapse tasks create` + `assign` でタスクボード登録後、`synapse send --notify` または `--silent` で委任 |
...
# 必須: 委任前にタスクボードに登録（タスクボード = チーム契約）
synapse tasks create "auth.py ユニットテスト作成" -d "JWT 認証のテスト" --priority 3
synapse tasks assign <task_id> Helper
