...
synapse claude --agent wise-strategist        # 保存済みエージェントIDで指定
synapse claude -A Alice                       # 表示名で指定（短縮フラグ）
synapse claude --agent wise-strategist --role "一時的なロール上書き"  # CLI引数で上書き可能
...
synapse claude -A Alice                       # 表示名で起動
synapse claude --agent wise-strategist --role "上書きロール"  # CLI引数が優先
```
...
`profile` の代わりに、保存済みエージェントの ID / 名前（例: `silent-snake`, `expert`）も指定できます。
...
# 保存済みエージェントID/名前で起動
synapse team start silent-snake gemini
...
```bash
# 追加（id は petname 形式: 小文字+ハイフン区切り、例: silent-snake, wise-strategist）
synapse agents add silent-snake --name expert --profile codex --role @./roles/reviewer.md --skill-set architect --scope project

# 一覧（TTYでは見やすいRich TUI表示）
synapse agents list

# 詳細確認
synapse agents show expert

# 削除
synapse agents delete silent-snake

# 起動時に使用
synapse codex --agent silent-snake              # プロファイルショートカットで使用
synapse spawn silent-snake                      # spawn で使用
synapse team start silent-snake happy-crab      # team start で使用
```
