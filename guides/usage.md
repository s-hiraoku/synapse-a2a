...
**デフォルト動作**: 1番目のエージェントが現在のターミナルを引き継ぎ（handoff）、2番目以降が新しいペインで起動します。**デフォルトで `--worktree` 分離が有効**です（`--no-worktree` でオプトアウト可能）。`--all-new` を指定すると、全エージェントが新しいペインで起動します（現在のターミナルはそのまま残ります）。

```bash
synapse team start <agent_spec1> <agent_spec2> ... [--layout split|horizontal|vertical] [--all-new] [--no-worktree]
```
...
親エージェントが常にライフサイクルを管理します: **spawn → send → evaluate → kill**（`--task` フラグで spawn → send をワンステップ化可能）
...
```bash
# ワンステップ: spawn + タスク送信（--task が READY 待機 → 自動送信）
synapse spawn gemini --name Tester --role "テスト担当" \
  --task "src/auth.py のユニットテストを書いて" --wait

# 結果を評価 — 不十分なら再送信（kill して再 spawn しない）
synapse send Tester "期限切れトークンのエッジケースも追加して" --wait

# 完了 — 必ず kill する（親がライフサイクルを管理）
synapse kill Tester -f
```

<details>
<summary>従来の手動ライフサイクル（spawn → list → send）</summary>
...
# 2つ目以降の spawn で自動的に tmux select-layout tiled が適用され、均等配置されます
# （team start だけでなく、個別の spawn でも自動タイルが有効）
...
synapse spawn gemini --name Tester --role "テスト担当" \
  --task "src/auth.py のユニットテストを書いて" --notify
synapse spawn claude --task-file /tmp/instructions.md --wait   # ファイルからタスク読み込み
synapse spawn codex --task "Fix lint errors" --task-timeout 60 --silent  # タイムアウト指定
...
- `synapse team start` は**デフォルトで worktree 分離が有効**です。`--no-worktree` でオプトアウトできます。
- `synapse spawn` で `--branch` / `-b` を指定すると **`--worktree` が自動的に有効化**されます。
...
- ブランチの自動マージ: `synapse kill` 実行時、worktree ブランチはデフォルトで親ブランチに自動マージされます（未コミットの変更は WIP コミットされます）。コンフリクト発生時はブランチが保持され警告が表示されます。`--no-merge` で自動マージをスキップできます:
  ...
  ```
- エージェントを停止せずにマージ: `synapse merge` でエージェントを稼働させたままブランチをマージできます（中間成果の統合に便利）:
  ```bash
  synapse merge my-claude              # エージェント稼働中にマージ
  synapse merge --all                  # 全 worktree ブランチをマージ
  synapse merge my-claude --dry-run    # プレビュー
  synapse merge my-claude --resolve-with gemini  # コンフリクト解消を委任
  ```
...
| STATUS | 現在のステータス (READY, WAITING, WAITING_FOR_INPUT, PROCESSING, SENDING_REPLY, RATE_LIMITED, DONE) |
...
```bash
synapse reply ["返信メッセージ"|--message-file PATH|--stdin] [--from <your_agent_id>] [--to SENDER_ID] [--list-targets] [--fail REASON]
```
...
| オプション | 説明 |
|-----------|------|
| `--message-file`, `-F` | ファイルから返信メッセージを読み込み（`-` で stdin）。バックティックやコードブロックを含む長文返信で shell 展開を回避 |
| `--stdin` | 標準入力から返信メッセージを読み込み |
...
**Note**: `synapse reply` は `synapse send` と同様の入力方法（positional / `--message-file` / `--stdin`）を提供しますが、返信は常に priority 3 / silent モードで送信されます。`--priority`、`--wait`/`--notify`/`--silent`、`--attach` 等の `synapse send` のフラグは意図的にミラーされていません。
...
# ファイルから長文返信を送信
synapse reply --message-file /tmp/review.md
...
- **CWD 絞り込み (local_only)** — `target: claude` のような型名ターゲットは、呼び出し元と同じワーキングディレクトリで動いているエージェントにのみ解決されます。別プロジェクトの同型エージェントに誤って送信されることはありません。0.31.0 で workflow_runner が `bare_type_same_dir_only=True` を強制するようになり、レジストリに同型の別ディレクトリ・エージェントが居ても workflow からは見えなくなりました ([#568](https://github.com/s-hiraoku/synapse-a2a/issues/568) / [#645](https://github.com/s-hiraoku/synapse-a2a/pull/645))。該当するエージェントが存在しない場合は `--auto-spawn` と組み合わせてその場で新規起動します (下記参照)。
- **`--auto-spawn` でスポーンされる子の権限フラグ** — workflow 経由で自動スポーンされる子エージェントには、プロファイルの `auto_approve.alternative_flags` の先頭が付与されます (例: codex なら `--dangerously-bypass-approvals-and-sandbox`)。バッチ実行されるワークフロー子は runtime permission prompt で止まらず走り切る必要があるため、対話用の workspace サンドボックス・デフォルトではなく sandbox を解除した形で起動します。対話的に `synapse spawn` / `synapse team start` で起動する場合は従来通りの workspace サンドボックス・デフォルト（codex 0.128+ では `-cdefault_permissions=":workspace"`、それ以前は `--full-auto`）です。
...
- **`GET /debug/waiting` と `synapse waiting-debug` (Phase 1 + 1.5, v0.28.1)** — 直近の WAITING 検知試行を ring buffer で保持するエンドポイントです。`path_used` (renderer / strip_ansi), `pattern_source` (primary / heuristic), `confidence`, `idle_gate_passed`, `renderer_on`, `new_data_hex_prefix`, `rendered_text_tail` などをその場で確認できます。`synapse status <agent> --debug-waiting` で aggregates 付きで pretty-print され、`synapse list` / `synapse status` は `(renderer: off)` と `renderer_available` フィールドでレンダラー初期化失敗を可視化します。複数エージェントを長時間サンプリングする場合は `synapse waiting-debug collect` / `report` (Phase 1.5 collection pipeline、詳細は [`docs/phase15-collection.md`](../docs/phase15-collection.md)) を利用してください。
...
- 完了時にトースト通知が表示されます
- 実行履歴は SQLite（`.synapse/workflow_runs.db`）に永続化され、サーバー再起動後も保持されます
