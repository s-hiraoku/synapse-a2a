...
| | 変更追跡 | 誰が何を変更したか記録 | 同上 |
| **学習** | Learning Mode | 2つの独立フラグで制御: `LEARNING_MODE_ENABLED` → Prompt Improvement セクション、`LEARNING_MODE_TRANSLATION` → JP→EN Learning セクション。どちらかが有効なら `learning.md` 注入と Tips が有効化。Response は通常フォーマット（セパレータなし）、構造化フォーマット（━━━ セパレータ、セクションヘッダー）はフィードバックセクション（Prompt Improvement, JP→EN Learning, Tips）のみに適用 | `SYNAPSE_LEARNING_MODE_ENABLED=true` |
