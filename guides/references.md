...
**Note**: `synapse status <agent>` の Recent Messages は、指定したエージェントが送信元または受信先として関与した履歴だけを表示します。0.31.0 以降、親プロセス側からの `synapse send` 観測も recent_messages に含まれ ([#659](https://github.com/s-hiraoku/synapse-a2a/issues/659) / [#662](https://github.com/s-hiraoku/synapse-a2a/pull/662))、タイムスタンプはミリ秒精度で一意化されます ([#661](https://github.com/s-hiraoku/synapse-a2a/issues/661) / [#666](https://github.com/s-hiraoku/synapse-a2a/pull/666))。送信側プレースホルダで `output_text` が上書きされる問題も解消済みです ([#660](https://github.com/s-hiraoku/synapse-a2a/issues/660) / [#663](https://github.com/s-hiraoku/synapse-a2a/pull/663))。
...
**動作**:
- 新しいペイン/ウィンドウでエージェントを起動（デフォルトで `layout="auto"`＝スポーンゾーンタイリング。最初の spawn でゾーンペインを作成し、以降の spawn はそのゾーン内でタイリング。2つ目以降の spawn で自動的に `tmux select-layout tiled` が適用され、ペインが均等配置される）
...
### 1.14.2 synapse cleanup

親が消失した子エージェント（オーファン）を検出して終了します。`synapse spawn` は親の `SYNAPSE_AGENT_ID` を子に `SYNAPSE_SPAWNED_BY` として伝搬し、レジストリに親子関係を記録します。`synapse cleanup` はその関係を元に、親エントリがレジストリから消えている・親 PID が死亡している子をオーファンと判定します。

```bash
synapse cleanup [AGENT] [--dry-run] [--force]
```

| 引数 | 必須 | 説明 |
|------|------|------|
| `AGENT` | No | 対象オーファンのエージェント ID / 名前（省略時はすべてのオーファン） |
| `--dry-run` | No | 実際には終了せず、オーファン一覧のみ表示 |
| `--force`, `-f` | No | 確認プロンプトをスキップ |

**動作**:

1. `AgentRegistry.get_orphans()` でオーファン候補を列挙
2. `--dry-run` なら一覧を表示して終了
3. そうでなければ各オーファンに対して `synapse kill` 相当の終了処理を実行し、レジストリから登録解除
4. `AGENT` を指定した場合、そのエージェントがオーファンでなければ拒否（通常終了は `synapse kill` を使用）

**例**:

```bash
synapse cleanup --dry-run                  # オーファン一覧をプレビュー
synapse cleanup                            # 全オーファンを終了（確認あり）
synapse cleanup -f                         # 確認なしで全オーファンを終了
synapse cleanup stale-codex                # 特定のオーファンのみ終了
```

**`synapse kill` との使い分け**:
- `synapse kill <agent>`: 通常のグレースフルシャットダウン（親が生存している正常なエージェント向け）
- `synapse cleanup <agent>`: 親が消失したオーファン専用（非オーファンには適用されない）

**`SYNAPSE_ORPHAN_IDLE_TIMEOUT` 環境変数**: 秒数を設定すると、`synapse list` 実行時に長時間 READY 状態のオーファンを自動回収します。デフォルトは無効（未設定）で、`synapse cleanup` は明示的なオプトインとして残ります。
...
### 1.15.1 synapse send-keys

実行中エージェントの PTY に生バイトを書き込みます。`synapse jump` で人間がターミナルに入って手動でキー入力するのではなく、CLI / スクリプトからプログラム的に同じことをやる escape hatch です（[#695](https://github.com/s-hiraoku/synapse-a2a/issues/695)、Refs [#694](https://github.com/s-hiraoku/synapse-a2a/issues/694)）。

主なユースケース: codex CLI の編集確認ダイアログ、モデル選択ピッカー、レート制限ダイアログなど、親エージェントが通常の A2A メッセージでは答えられない TUI ダイアログでスタックしたエージェントの解除。
...
|------|------|------|
| `TARGET` | Yes | エージェント名、ID、type-port、またはタイプ |
| `DATA` | No | 書き込むバイト列（例: `'a'`, `'y\r'`, `'\x1b'`）。`--enter` を指定する場合は省略可 |
| `--enter` | No | 末尾に CR (`\r`) を submit シーケンスとして付加 |
| `--no-escape` | No | `DATA` を `unicode_escape` でデコードせず literal として扱う（デフォルトは `\r` / `\x1b` 等を解釈） |
| `--json` | No | HTTP レスポンスを JSON で出力（`{ok, bytes_written, submit_seq_sent}`） |

**例**:

```bash
synapse send-keys codex 'y' --enter             # 編集確認ダイアログで Yes
synapse send-keys codex '\x1b'                  # ESC でモデルピッカーを閉じる
synapse send-keys claude '' --enter             # Enter のみを送信
synapse send-keys codex 'literal\r' --no-escape # \r をそのまま literal バイトとして送る
```

内部的には `POST /pty/write`（[2.3.3](#233-post-ptywrite)）を呼び出します。エンドポイントは `require_auth` で保護されており、`/tasks/{id}/cancel` や `/tasks/{id}/permission/approve` と同じトラストモデルです。
...
> コンセプト・YAML スキーマ・5 パターンの使い分け・拡張手順の詳細は
> [`multi-agent-patterns.md`](multi-agent-patterns.md) を参照してください。
> 実用サンプルは [`examples/patterns/`](../examples/patterns/) にあります。
...
# --timeout <seconds>    /debug/waiting へのHTTP timeout（default: 5.0）
...
# --json                 集計を JSON で出力（stdout）
# --out <path>           集計を JSON として指定ファイルに書き出し、stdout は空に保つ（cron / スクリプト向け）
```

- `collect` は各エージェントの `http://localhost:<port>/debug/waiting` を **5 秒 timeout**（`--timeout` で上書き可能、v0.28.2 で 3 秒 → 5 秒に引き上げ）で叩き、取得した snapshot に `agent_id` / `agent_type` / `port` / `collected_at` を付与して JSONL に追記します。取得に失敗したエージェントについては stderr に 1 件 warning を出して skip します（legacy な pre-0.28.0 バイナリで起動中のエージェントは `/debug/waiting` を公開していないので、この挙動が正常です）。`HOME` を上書きして走らせるテスト／代替プロファイルでもデフォルト出力先が期待通り `<HOME>/.synapse/waiting_debug.jsonl` に解決されます（v0.28.2 で遅延解決化）。
...
| POST | `/pty/write` | 生バイトをエージェント PTY に書き込む escape hatch（[#695](https://github.com/s-hiraoku/synapse-a2a/issues/695)）。`require_auth` で保護され、`/tasks/{id}/cancel` / `permission/approve` と同じトラストモデル。詳細は [2.3.3](#233-post-ptywrite) |
...
### 2.3.3 POST /pty/write

エージェントが TUI ダイアログ（codex CLI の編集確認、モデル選択ピッカー、レート制限ダイアログなど）でスタックし、通常の A2A メッセージでは答えられないときに、親側からプログラム的に PTY へ生バイトを書き込むための escape hatch です（[#695](https://github.com/s-hiraoku/synapse-a2a/issues/695)、Refs [#694](https://github.com/s-hiraoku/synapse-a2a/issues/694)）。CLI ラッパーは [`synapse send-keys`](#1151-synapse-send-keys)。

セキュリティ: `require_auth` で保護されており、`/tasks/{id}/cancel` や `/tasks/{id}/permission/approve` と同じトラストモデルです。

**リクエスト**:

```http
POST /pty/write HTTP/1.1
Host: localhost:8100
Content-Type: application/json
...
| フィールド | 型 | 必須 | 説明 |
|-----------|-----|------|------|
| `data` | string | Yes | PTY に書き込む文字列。制御文字はあらかじめデコード済みであること（CLI 側 `synapse send-keys` がデフォルトで `unicode_escape` 展開する） |
| `submit_seq` | string \| null | No | 末尾に追加の submit シーケンスを書き込みたい場合に指定（例: `"\r"`） |
...
```json
{
  "ok": true,
  "bytes_written": 2,
  "submit_seq_sent": false
}
```

| フィールド | 型 | 説明 |
|-----------|-----|------|
| `ok` | bool | コントローラの `write()` が成功したか |
| `bytes_written` | int | `data` の UTF-8 バイト数（`submit_seq` は含まない） |
| `submit_seq_sent` | bool | `submit_seq` が指定されていたか |

コントローラが存在しない場合は `503 Service Unavailable`、`data` が string でない場合は `400 Bad Request` を返します。
