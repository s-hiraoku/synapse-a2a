...
**Views:** Hash-routed SPA with three views — `#/` (Canvas spotlight), `#/history` (grid + live feed + agent messages), and `#/system` (dedicated system panel). Navigation via sidebar (fixed on desktop, hamburger drawer on mobile); view state preserved across SSE reconnects.
