...
| **LLM Wiki** | Knowledge accumulation layer inspired by Karpathy's LLM Wiki pattern. Agents build and maintain a structured, interlinked Markdown knowledge base with frontmatter metadata, `[[wikilink]]` cross-references, and confidence scores. Two scopes: `.synapse/wiki/` (project) and `~/.synapse/wiki/` (global). CLI: `synapse wiki ingest/query/lint/status`. Canvas Knowledge view at `#/knowledge`. MCP instruction `synapse://instructions/wiki`. Config: `wiki.enabled` (default `true`). See [LLM Wiki Design](docs/design/llm-wiki.md) |
