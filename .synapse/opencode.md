...
- `--from`: Your agent type (e.g., `opencode`, `claude`, `codex`, `gemini`, `copilot`)
- Required in sandboxed environments (like Codex)

Example - Question received:
  A2A: What is the project structure?
Reply with:
  synapse reply "The project has src/, tests/, docs/ directories..." --from opencode
...
Parameters:
- `--from, -f`: Your agent type (e.g., `opencode`) - **always include this**
