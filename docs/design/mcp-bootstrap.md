...
- PTY bootstrap の自動切り替え — `has_mcp_bootstrap_config()` により Claude Code, Codex, Gemini CLI, OpenCode, Copilot で MCP 設定検出時に full instructions の代わりに最小 PTY bootstrap を送信（approval は維持）
...
つまり、現時点の実装は「instruction resources の配布」「bootstrap_agent による runtime context 取得」「MCP 検出時の最小 PTY bootstrap 自動切り替え（approval 維持）」までである。
...
[mcp_servers.synapse]
command = "/path/to/uv"
args = ["run", "--directory", "/path/to/repo", "python", "-m", "synapse.mcp"]
```
...
```bash
/path/to/uv run --directory /path/to/repo python -m synapse.mcp
```

> **Note:** `--agent-id`, `--agent-type`, `--port` は `$SYNAPSE_AGENT_ID` 環境変数から自動解決されるため、明示的に指定する必要はない。
...
- MCP 設定が存在する場合、full instructions の代わりに最小 PTY bootstrap を送信する（approval は維持）
- MCP 設定がない場合は従来通り PTY フォールバック（full instructions）で動作する
