...
> **運用注記 (優先ルールの明文化):** 既存の「`plugins/synapse-a2a/skills/`
> 配下を正本として `.claude/skills/` / `.agents/skills/` / `.gemini/`
> へ同期する」ルールは引き続き既定。本 RFC はそれを置き換えるのではなく
> **狭い例外**として Local dev skill のみ直接編集を許可する。具体的には:
>
> 1. **既定 (Published / Workflow-autogen)**: `plugins/synapse-a2a/skills/`
>    または `.synapse/workflows/` の正本だけを編集し、`sync-plugin-skills`
>    で `.claude/skills/` と `.agents/skills/` に伝播させる。これら派生
>    ディレクトリへの直接編集は禁止。
> 2. **例外 (Local dev)**: SKILL.md の frontmatter に明示的な
>    `local-dev: true` マーカーを持ち、かつ `plugins/synapse-a2a/skills/`
>    にも `.synapse/workflows/` にも対応エントリが存在しない skill のみ、
>    `.claude/skills/<name>/` または `.agents/skills/<name>/` を正本として
>    直接編集してよい。
> 3. **競合解消**: `sync-plugin-skills` は `local-dev: true` マーカー
>    付きエントリをスキップする (上書きしない)。マーカーがない派生
>    ファイルがローカルで変更されていた場合は同期時に上書きされるため、
>    変更を残したい場合は `local-dev: true` を付けるか正本側に戻す。
>
> マーカーの具体的な spec とツール側の実装は後続タスクで整備する。
...
- [ ] `.claude/skills/` と `.agents/skills/` に `GENERATED.md` もしくは
      ...
      SKILL.md, `local-dev: true` マーカー付き) は手編集可であることを
      明示する。あわせて `local-dev: true` マーカーの仕様化と
      `sync-plugin-skills` 側のスキップ実装を行う (本 RFC の「運用注記」
      で示した優先ルールの実装)。
