...
### バックチャネルメッセージ（計画中 / 未実装）

ランタイム中に動的な通知を PTY に出力せず TaskStore にのみ保存する **バックチャネルメッセージ** は未実装です。

> **Note**: `synapse send --silent` はレスポンスモードの一つ（fire-and-forget）であり、メッセージ自体は PTY に送信されます。ここで言うバックチャネルメッセージは、PTY を経由せず TaskStore のみに保存される別の概念です。

将来的に導入する場合の想定:
- TaskStore に保存されるが PTY には書き込まれない
- AI エージェントは `/tasks?context_id=backchannel` で取得可能
