...
### 5. エージェント無知の原則（Agent Ignorance Principle）

**エージェントはコラボレーションの詳細を知る必要がない。**

Synapse A2A の設計において最も重要な原則の一つ。エージェントは「スタンドアロンで動作している」という錯覚を維持したまま、実際には他のエージェントと協調できる。

```text
┌─────────────────────────────────────────────────────────────────┐
│  エージェントが知るべきこと（最小限）                            │
├─────────────────────────────────────────────────────────────────┤
│  ✅ メッセージを受信した（A2A: <message>）                       │
│  ✅ 返信が必要な場合は synapse reply で返す                      │
│  ❌ 誰から来たか（task_id, sender_id）                          │
│  ❌ 通信プロトコルの詳細（UDS, HTTP, Reply Stack）              │
│  ❌ ルーティングの仕組み                                        │
└─────────────────────────────────────────────────────────────────┘
```

**Synapse（A2A層）が担当すること:**

- 送受信の制御
- Reply Stack によるルーティング管理
- task_id, sender_id の追跡
- 適切なエンドポイントへの配送
- 通信プロトコルの選択（UDS/HTTP）
...
- メッセージ内容の理解と処理
- タスクの実行
- `synapse reply` での返信（内容のみ指定）

この設計により:
- **シンプルなインターフェース**: エージェントは複雑なプロトコルを理解する必要がない
- **疎結合**: 通信層の変更がエージェントに影響しない
- **エラー耐性**: ルーティングエラーは Synapse 層で処理される
- **将来の拡張性**: 新しい通信方式を追加してもエージェントは変更不要

```mermaid
flowchart TB
    subgraph Agent["エージェント（無知）"]
        Receive["A2A: msg を受信"]
        Process["タスク実行"]
        Reply["synapse reply 'result'"]
    end

    subgraph Synapse["Synapse A2A 層（全知）"]
        Router["ルーティング管理"]
        Stack["Reply Stack"]
        Transport["UDS / HTTP"]
        TaskMgmt["Task ID 管理"]
    ...
    Router --> Receive
    Reply --> Stack --> Transport

    style Agent fill:#fff3e0
    style Synapse fill:#e1f5fe
```

### 6. 最小限の表示（Minimal Visibility）
...
| エージェント負担 | Synapse 層で処理 | エージェントに複雑さを押し付ける |
| 検証 | 実際に動作するもの | 理論のみ |

**エージェント無知の原則チェックリスト:**

新機能を追加する際は以下を確認:
- [ ] エージェントが新しい構文やフォーマットを学ぶ必要があるか？ → 最小限に
- [ ] task_id や sender_id をエージェントが扱う必要があるか？ → Synapse で処理
- [ ] 返信時にエージェントが宛先を指定する必要があるか？ → Reply Stack で自動化
