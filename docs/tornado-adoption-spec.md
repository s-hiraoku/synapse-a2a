...
- [5. ワークフロー実行パターン: Manager + TaskBoard](#5-ワークフロー実行パターン-manager--taskboard)
...
Synapse A2A は P2P アーキテクチャであり、中央集権的なワークフローエンジンはプロジェクト哲学に反します。代わりに、manager エージェント（delegate-mode）が既存の TaskBoard を監視し、各エージェントに `synapse send` でタスクを指示する **Kanban パターン** を採用します。

```
manager (delegate-mode)                各エージェント
  │                                          │
...
| 2 | Dev-Review Loop | **不採用（専用エンジン）** | manager + TaskBoard の Kanban パターンで代替 |
...
| 7 | Session Persistence | **不採用** | manager の LLM が文脈を保持。TaskBoard 自体が SQLite で永続化済み |
| 8 | SDK Adapter Pattern | **不採用** | Synapse は PTY ラッピング方式であり SDK 不要 |
| 9 | Git Context | **不採用** | エージェントが自分で git コマンドを実行できる |
| 10 | Task Decomposition | **不採用（専用機能）** | manager がタスク分解し TaskBoard に登録する運用で代替 |
...
## 5. ワークフロー実行パターン: Manager + TaskBoard
...
```
manager (delegate-mode, スキルで動作ルールを定義)
  │
  ...
  │   └─ synapse send <agent> "タスクの内容" --silent
  │
  ├─ 完了通知を受けたら（または定期確認で）
...
### 5.2 manager のスキル定義（例）

manager に渡すスキル指示の例:

```
あなたは TaskBoard を監視する manager です。
...
時刻    TaskBoard                              動作
────    ─────────                              ────
T0      task-1: pending (available)            manager → dev-1 に assign
...
T2      task-1: completed                      dev-1 完了 → task-2 が unblock
        task-2: pending (available)            manager → dev-1 に assign
        task-3: pending (blocked)
...
T4      task-2: completed                      dev-1 完了 → task-3 が unblock
        task-3: pending (available)            manager → reviewer に assign

T5      task-3: in_progress (reviewer)         reviewer がレビュー中

T6a     task-3: completed                      全完了 → manager が報告
    or
T6b     task-3: failed (reason: "テスト不足")   manager が判断:
...
T0: get_available_tasks() → [task-1 (pri=5), task-2 (pri=3)]
    manager: task-1 を dev-1 に、task-2 を dev-2 に assign（並列実行）

T1: task-1 完了 → task-3 が unblock
    get_available_tasks() → [task-3 (pri=5)]
    manager: task-3 を dev-1 に assign（優先度が高い）

T2: task-2 完了 → task-4 が unblock
    manager: task-4 を dev-2 に assign
```
...
| Collaborative | OK | manager + worker の Kanban パターンで協調を実現 |
