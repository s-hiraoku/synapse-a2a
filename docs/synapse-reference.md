...
- **Copilot**: Timeout strategy, 0.5s. `write_delay: 0.5` for Ink TUI rendering. `bracketed_paste: true` for Ink's `usePaste` hook. Short single-line A2A messages can be typed character-by-character instead of pasted, while longer messages still use paste. Inside tmux, typed input uses a slower minimum per-character delay so the multiplexer does not batch characters into a burst. Multiline/file-reference sends use `long_submit_settle_delay` and `long_submit_retry_delay` before the initial submit and retry submit. `submit_retry_delay: 0.15` remains the default short-message Enter safety net, plus bounded submit confirmation (`submit_confirm_timeout`, `submit_confirm_poll_interval`, `submit_confirm_retries`) when injected text appears to remain pending. `submit_fallback_sequences: ["\n", "\x13", "\x1b\r"]` — on each confirmation retry the controller prefers `Ctrl+S` when the footer advertises `ctrl+s run command`, then falls back through LF and `Esc+CR` before returning to the original `submit_sequence`.

For `response_mode=wait` and `response_mode=notify`, Synapse completes the sender-side task via structured reply artifacts. Completion finalization now prefers the PTY output delta captured since task start rather than the raw terminal tail, reducing reply corruption from transient TUI lines such as update banners, permission rows, and status bars. For Copilot specifically, `clean_copilot_response()` strips Ink TUI artifacts (spinners, box-drawing borders, status bar, and input echo) from the captured delta before building the reply. Quota-exhaustion output such as `402 You have no quota` is classified as a failed task instead of being returned as a normal reply.

In server mode (when `agent_name` is set), startup/runtime logging is redirected from stderr to file-only to prevent log output from corrupting the agent TUI.
