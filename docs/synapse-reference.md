...
**Stuck-Agent Watchdog**: `synapse watchdog check` (#646) is a one-shot, read-only command that scans every live agent and surfaces stuck-state suspicions. Heuristics: `RATE_LIMITED > 30m`, `SENDING_REPLY > 60s`, `WAITING` with the codex CLI rate-limit reminder dialog visible in the PTY tail (alarm `rate_limit_dialog`, #691/#692), `WAITING` with the codex CLI edit-confirmation dialog ("Would you like to ...") visible in the PTY tail (alarm `edit_confirmation_dialog`, #707), `PROCESSING > 30m` with no outbound A2A in the last 10m, and "spawn never ready" (registered > 60s ago, < 5m, status != READY). The dialog-detection alarms can be cleared without `synapse jump` via `synapse send-keys <target> '\x1b'` (ESC the rate-limit dialog) or `synapse send-keys <target> 'y' --enter` (approve the edit-confirmation dialog). `--alarm-only` filters to alarm rows; `--json` emits a programmatic array (each report adds `alarm_reason` alongside `alarm`). Stage 2-4 (background daemon, push notifications, multi-watchdog locking, `synapse list --watch` integration, automatic recovery) are future work.

**Programmatic agent JSON view**: `synapse list --json` and `synapse status <target> --json` share six canonical fields — `agent_id`, `status`, `current_task_preview`, `task_received_at`, `uptime_seconds`, `input_required_tasks` — produced by `synapse/commands/_agent_view.py:build_agent_json_view()` (#708). The `status` value is the machine-readable enum without the ` [ORPHAN]` annotation that decorates the human `synapse list` table; consumers should branch on `is_orphan` (list view only) instead of substring-matching the status string.
...
**WAITING → input_required (A2A)**: When an agent enters WAITING status, the A2A task status is mapped to `input_required` per the Google A2A spec. The child notifies its parent with both the legacy permission metadata and a structured `permission_escalation` block that the parent-side Approval Gate can auto-dispatch. In `synapse send --wait`, the caller keeps polling until the parent intervenes or the intervention timeout expires. Manual approve/deny via the permission endpoints remains available. See [docs/permission-detection-spec.md](permission-detection-spec.md).
