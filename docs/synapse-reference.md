...
- **Copilot**: Timeout strategy, 0.5s. `write_delay: 0.5` for Ink TUI rendering. `bracketed_paste: true` for Ink's `usePaste` hook. Short single-line A2A messages can be typed character-by-character instead of pasted, while longer messages still use paste. `submit_retry_delay: 0.15` is the first Enter safety net, plus bounded submit confirmation (`submit_confirm_timeout`, `submit_confirm_poll_interval`, `submit_confirm_retries`) when injected text appears to remain pending. `submit_fallback_sequences: ["\n", "\x13", "\x1b\r"]` — on each confirmation retry the controller prefers `Ctrl+S` when the footer advertises `ctrl+s run command`, then falls back through LF and `Esc+CR` before returning to the original `submit_sequence`.
