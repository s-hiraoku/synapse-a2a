...
- **Copilot**: Timeout strategy, 0.5s. `bracketed_paste: true` for Ink's `usePaste` hook. Copilot sends always use bracketed paste plus Enter — typed input and `Ctrl+S` fallbacks are not used. Three PTY-level fixes ensure reliable Enter key execution after paste injection: (1) `tty.setraw(slave_fd)` is called on the PTY slave before spawning the child process — the default line discipline includes ICRNL which translates `\r` to `\n`, and Ink-based TUIs expect literal `\r` for Enter; (2) `termios.tcdrain()` is called after writing the bracketed paste sequence to ensure the OS delivers the complete paste (including the close marker `ESC[201~`) atomically before CR is sent — without this, a large write may be split across reads, confusing Ink's paste boundary detection; (3) a 150 ms **settle delay** (`_COPILOT_PASTE_ECHO_SETTLE`) runs after paste echo is detected, giving React's asynchronous `setState` time to commit the pasted text into the internal input buffer before CR arrives. After pasting, Synapse uses **adaptive paste echo wait** instead of a fixed `write_delay`: it polls PTY output until Copilot's Ink TUI re-renders (React state update after `usePaste`), then sends Enter. Falls back to `write_delay` (0.5 s) if no echo is detected within 3 s. Before the main confirmation retry loop, a short **nudge delay** (0.1 s normal, 0.2 s for long messages) fires an extra Enter to handle cases where the first Enter arrives before the TUI is ready. Bounded submit confirmation (`submit_confirm_timeout`, `submit_confirm_poll_interval`, `submit_confirm_retries`) then verifies that Copilot actually cleared the prompt. Submit confirmation stays pending while Copilot still shows the original prompt text, file-reference markers, or placeholders such as `[Paste #1 - 12 lines]` and `[Saved pasted content to workspace ...]`, including when the same placeholder label is reused across consecutive sends. Long multiline or file-reference sends use a larger confirmation budget (`long_submit_confirm_timeout`, `long_submit_confirm_retries`).

For `response_mode=wait` and `response_mode=notify`, Synapse completes the sender-side task via structured reply artifacts. Completion finalization now prefers the PTY output delta captured since task start rather than the raw terminal tail, reducing reply corruption from transient TUI lines such as update banners, permission rows, and status bars. TUI response cleaning (`clean_copilot_response()`) now runs for **all agents** (not just Copilot), stripping Ink TUI artifacts (spinners, box-drawing borders, status bar, and input echo) from the captured delta before building the reply. The `strip_ansi()` helper uses three-stage removal (full ANSI sequences, orphaned SGR fragments, bare SGR fragments) to handle cases where `\r` overwrites the ESC prefix. Quota-exhaustion output such as `402 You have no quota` is classified as a failed task instead of being returned as a normal reply.
