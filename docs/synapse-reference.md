...
- **Copilot**: Timeout strategy, 0.5s. `write_delay: 0.5` for Ink TUI rendering. `bracketed_paste: true` for Ink's `usePaste` hook. Copilot sends always use bracketed paste plus Enter — typed input and `Ctrl+S` fallbacks are not used. Synapse pastes the message, presses Enter, and then uses bounded submit confirmation (`submit_confirm_timeout`, `submit_confirm_poll_interval`, `submit_confirm_retries`) to verify that Copilot actually cleared the prompt. Submit confirmation stays pending while Copilot still shows the original prompt text, file-reference markers, or placeholders such as `[Paste #1 - 12 lines]` and `[Saved pasted content to workspace ...]`. Long multiline or file-reference sends use a larger confirmation budget (`long_submit_confirm_timeout`, `long_submit_confirm_retries`).
