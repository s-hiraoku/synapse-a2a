...
- **Added**: `synapse watchdog check` now also detects the codex CLI edit-confirmation dialog ("Would you like to make the following edits?") in PTY tail content and surfaces an `edit_confirmation_dialog` alarm (#707). Like the existing `rate_limit_dialog` detection (#691), the check is gated on `status == WAITING` so stale PTY content cannot trip the alarm in `READY` / `PROCESSING`. Pair with [`synapse send-keys`](reference/cli.md#send-keys) to programmatically answer the dialog once it fires.
- **Changed**: `synapse status --json` and `synapse list --json` now share six canonical fields rendered by the new `synapse/commands/_agent_view.py` helper: `agent_id`, `status`, `current_task_preview`, `task_received_at`, `uptime_seconds`, and `input_required_tasks` (#708). Previously `synapse list --json` did not emit `uptime_seconds` and `synapse status --json` derived `task_received_at` ad-hoc; both now resolve through the shared `build_agent_json_view()` so AI/programmatic callers can rely on identical field shapes regardless of which command they query. Status annotations like `[ORPHAN]` are stripped from the machine-readable `status` value while remaining in the human Rich TUI column.
...
- **Fixed**: `synapse spawn --worktree` no longer dies with `RuntimeError: Worktree directory already exists` when an auto-generated name (`safe-goat`, `firm-crow`, …) collides with a stale `.synapse/worktrees/<name>/` directory or a leftover `worktree-<name>` branch (#704). The auto-name path retries up to 8 candidates, treating both directory and branch existence as collisions, and also retries when `git worktree add` itself fails with a path/branch collision marker (TOCTOU window for parallel `synapse spawn` calls). Explicit `--name <foo>` collisions still fail immediately so the caller's chosen name is never silently swapped, but the error now includes a `git worktree remove <path>` (or `git branch -D <branch>`) cleanup hint. The exhaustion error points at `git worktree list` / `git worktree prune`.
- **Refactored**: Consolidate the duplicated `_is_agent_alive` PID + port-probe + retry-with-sleep ladder that lived in both `synapse/commands/list.py` and `synapse/commands/status.py` into the shared `synapse.commands._agent_view.is_agent_alive` (#710). Drops a dead `data.update({...})` block in `StatusCommand._render_json` and introduces a shared `ORPHAN_STATUS_SUFFIX` constant so the writer (`list.py`) and stripper (`_agent_view.strip_status_annotations`) cannot drift on the literal annotation. Pure refactor — full pytest passes.
- **Documentation**: README, `docs/synapse-reference.md`, and `guides/{usage,references,troubleshooting}.md` document the six canonical JSON fields and list both `rate_limit_dialog` and `edit_confirmation_dialog` in the watchdog heuristic enumeration (was 4 alarms, now 6). The `synapse-a2a` skill (canonical at `plugins/synapse-a2a/skills/synapse-a2a/`, mirrored to `.agents/skills/` and `.claude/skills/`) now surfaces the `canvas_post` MCP tool that was already exposed by the MCP server but missing from the skill's tool list.

### v0.33.0

The 0.33.0 release tackles the **autonomous-completion** rough edges surfaced during 0.32.0 dogfooding. New `synapse send-keys <agent> <data>` CLI and `POST /pty/write` endpoint let a parent agent programmatically respond to a child's TUI dialog (codex CLI edit-confirmation, model picker, rate-limit prompt) without resorting to `synapse jump` (#695). Two observability bugs are fixed alongside: `current_task_preview` no longer persists past task completion (with a deliberate deferred-clear when the agent is mid-`SENDING_REPLY`, so PR #644's preview-during-send semantic is preserved) (#689), and `synapse reply --to <sender>` now reports whether the reply stack is empty or has different senders instead of the generic "No reply target" message (#690). The deferred-clear flag was rewritten from a list-as-mutable-bool wrapper to `nonlocal` for readability (#700).

- **Added**: `synapse send-keys <agent> <data>` CLI and `POST /pty/write` HTTP endpoint for writing raw input bytes into a controlled CLI's PTY (#695). Decodes Python escape sequences (`\r`, `\x1b`, …) by default; `--no-escape` keeps text literal; `--enter` appends a carriage return as the submit sequence. Endpoint is gated by the same `require_auth` dependency as `/tasks/{id}/cancel` and `/tasks/{id}/permission/approve`.
- **Fixed**: Clear stale `current_task_preview` / `task_received_at` metadata when A2A tasks reach completed, failed, or canceled terminal states outside the normal finalization path. While the agent is in `SENDING_REPLY` the clear is deferred and applied automatically when the agent next leaves that sub-state, so the preview never persists indefinitely even if the terminal transition lands during outbound reply send (#689).
- **Fixed**: `synapse reply --to <sender_id>` now reports whether the reply stack is empty or contains different sender IDs when the requested sender is missing, while preserving the existing persisted fallback behavior (#690).
- **Refactored**: `_pending_terminal_preview_clear` in the A2A router uses a `nonlocal` bool instead of a list-as-mutable-bool wrapper. Equivalent behavior with clearer Python idiom (#700).
...
- **Added**: `synapse watchdog check` detects Codex CLI rate-limit model-switch dialogs from PTY debug output and surfaces a `rate_limit_dialog` alarm (#691). Gated on `status == WAITING` so stale PTY content cannot trip the alarm in `READY` / `PROCESSING`; the `/debug/pty` fetch is skipped for non-`WAITING` agents to avoid per-tick HTTP overhead. Recovery: pair with [`synapse send-keys`](reference/cli.md#send-keys) to dismiss the dialog programmatically.
...
- **Added**: `.github/PULL_REQUEST_TEMPLATE.md` enforcing the `Closes #<num>` / `Refs #<num>` keyword convention so issues auto-close on merge instead of orphaning. CLAUDE.md gains a "Closing shipped issues" section documenting backlog triage (#670).
