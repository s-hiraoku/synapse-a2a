...
!!! tip "Recovery for wedged TUI dialogs (#695)"
    When an alarm fires because the controlled CLI is wedged on a TUI dialog the A2A pipeline cannot answer (codex edit-confirmation, model picker, rate-limit dialog), reach for [`synapse send-keys`](../reference/cli.md#send-keys) — it writes raw bytes straight into the agent's PTY via `POST /pty/write`, bypassing the readiness gate. Use it only after `synapse send` / `synapse interrupt` have failed, since it skips every safeguard the Task pipeline normally enforces.
