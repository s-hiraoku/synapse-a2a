...
| **dev-issue** | `/dev-issue <number>` — bootstrap a new issue implementation: fetches the issue and related closed PRs in parallel, greps the repo for code hotspots, infers a branch prefix from labels and a slug from the title, writes a structured task brief to `/tmp/issue<num>-task.md`, creates a fresh branch from the latest `origin/main`, and spawns a codex agent (override with `--solo`, or use `--dry-run` for the brief only). The summary now also recommends the appropriate post-implementation skill (`/post-impl-codex` for default, `/post-impl-claude` or `/post-impl` for `--solo`, none for `--dry-run`) so the docs/skills/site-sync chain isn't silently dropped after implementation ([#665](https://github.com/s-hiraoku/synapse-a2a/issues/665)). See [#643](https://github.com/s-hiraoku/synapse-a2a/issues/643). |
