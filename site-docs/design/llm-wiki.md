...
| learning | 実行結果から学んだ教訓 | `learning-pty-timeout-handling.md` |
...
| Field | Description |
|---|---|
| `source_files` | List of repository file paths this page was derived from |
| `source_commit` | Short git commit hash at the time the page was last synced |

When `source_files` and `source_commit` are present, `wiki lint` and `wiki status` can detect **stale pages** — pages whose tracked source files have changed since the recorded commit. The `wiki refresh` command updates the `source_commit` field after review.
...
- ページ数、ソース数、最終更新日時
- 健康度スコア（lint 結果ベース）
- Stale ページ数（`source_files`/`source_commit` ベースの変更検出）
...
**CLI の責務（確定的）:**
- `source_files` と `source_commit` を持つページを走査
- `git diff` で `source_commit` 以降の変更を検出
- stale ページ一覧を表示
- `--apply` 指定時に `source_commit` を HEAD に更新

**エージェントの責務（知的）:**
- stale ページの内容をソースファイルの変更に合わせて更新
...
**CLI の責務（確定的）:**
- Wiki ディレクトリ構造を作成（`pages/`, `sources/`, `schema.md`, `index.md`, `log.md`）
- スケルトンページを生成（`synthesis-architecture.md`, `synthesis-patterns.md`）
- 既存ページはスキップ
...
GET /api/wiki/enabled                    # Wiki 機能の有効/無効チェック
GET /api/wiki/{scope}/pages/{page}       # ページ内容（Markdown）
GET /api/wiki/stats?scope=project|global # 統計情報
GET /api/wiki/graph?scope=project|global # ページ間リンクの Mermaid グラフ
```
...
- type フィルタ（entity, concept, decision, comparison, synthesis）
- テキスト検索（ページタイトル + サマリ）
...
- [ ] `synapse/wiki.py` — CLI コマンド実装（ingest, query, lint, status）
- [ ] Wiki ディレクトリ自動作成ロジック
- [ ] `schema.md` テンプレート
