...
!!! note "Worktree-aware path matching (v0.20+)"
    Prior to v0.20, Synapse treated worktree directories as unrelated projects and blocked sends by default. The working-directory check now recognizes `.synapse/worktrees/` paths and allows sends automatically. `--force` is still available for genuinely cross-project messaging.
