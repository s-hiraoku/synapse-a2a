...
[![Tests](https://img.shields.io/badge/tests-2990%20passed-brightgreen.svg)](#テスト)
...
| **Canvas** | エージェント共有のビジュアル出力面。Mermaid ダイアグラム、テーブル、チャート、コード、diff など 22 フォーマットをブラウザ UI でレンダリング。6 種のレイアウトテンプレート: `briefing`, `comparison`, `dashboard`, `steps`, `slides`, `plan`。**Plan Card** テンプレートは Mermaid DAG + ステップリストでタスク計画を可視化し、Task Board 連携 (`synapse tasks accept-plan`) で計画を実行に移行。`synapse canvas serve` (ポート 3000) |
...
メッセージは、オプションの送信者識別情報と返信期待マーカーを含むプレフィックス付きで、エージェントの PTY に送信されます：
...
- **Ghostty のフォーカス**: Ghostty は AppleScript を使用して現在フォーカスされているウィンドウやタブを対象にします。`spawn` や `team start` コマンドの実行中にタブを切り替えると、意図しないタブにエージェントがスポーンされる可能性があります。コマンドが完了するまで操作を控えてください。
