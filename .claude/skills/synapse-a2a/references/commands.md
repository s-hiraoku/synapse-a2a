...
For `--wait` and `--notify`, reply artifacts are built from the PTY output delta captured since task start rather than the raw terminal tail. For all agents, Synapse applies three-stage ANSI stripping (full escape sequences, orphaned SGR fragments, and bare SGR fragments) to clean TUI output. For Copilot specifically, Synapse also strips Ink TUI artifacts from the captured delta. Copilot uses bracketed paste plus Enter; after pasting, Synapse polls PTY output for the paste echo (up to 3 s) before sending Enter, ensuring React's Ink render cycle has completed. Submit confirmation uses adaptive nudge timing (0.1 s for normal messages, 0.2 s for long messages) and stays pending while the visible prompt still shows the original text, file-reference markers, or placeholders such as `[Paste #1 - 12 lines]` and `[Saved pasted content to workspace ...]`. Quota errors such as `402 You have no quota` are surfaced as failed tasks instead of normal replies.
