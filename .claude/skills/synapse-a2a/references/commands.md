...
For `--wait` and `--notify`, reply artifacts are built from the PTY output delta captured since task start rather than the raw terminal tail. For Copilot, Synapse also strips common Ink TUI artifacts from the captured delta. Copilot sends stay on bracketed paste plus Enter instead of typed input or `Ctrl+S` fallbacks. Submit confirmation stays pending while Copilot still shows the original prompt text, file-reference markers, or placeholders such as `[Paste #1 - 12 lines]` and `[Saved pasted content to workspace ...]`. Quota errors such as `402 You have no quota` are surfaced as failed tasks instead of normal replies.
